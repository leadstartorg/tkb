...
independently owned Jani-King franchisee in Lilburn, GA.
...
| # | What | Where |
|---|------|-------|
| 1 | **Street address** `[Street address]` | `build.py` `ADDRESS_1`. Also confirm ZIP — Lilburn uses both 30047 and 30048 |
| 2 | Phone `(770) 000-0000` | all pages, `build.py` `PHONE_DISPLAY` / `PHONE_HREF`, `main.js` fail message |
| 3 | Email `info@tkbventures.com` | all pages, `build.py` `EMAIL` |
| 4 | Social URLs set to `#` | footer |
| 5 | Form endpoints | add `data-endpoint="…"` to each `<form>` |
| 6 | ZIP coverage list | `main.js` → `SERVICE_PREFIXES` |
| 7 | Hero image (AI approved) | `.hero { --hero-photo: url("assets/img/…"); }` |
| 8 | Operations image (AI approved) | `about.html`, first section |
| 9 | Owner headshot (AI approved) | `about.html`, owner bio |
| 10 | Privacy and terms copy | `privacy.html`, `terms.html` — needs counsel |

### Images

Carolyn approved AI-generated imagery provided it reads as real photography.
Three slots are stubbed with labelled placeholder boxes, each with the exact
`<img>` tag to paste in an HTML comment directly above it. I can't generate
images in this session, so the boxes ship as-is.
